...
import UnauthenticatedError from "../errors/unauthorized"
...
.sort({ createdAt: -1 })
.select(
"title description img author tags views likesCount"
)
.populate({
path: "author",
select: "name profileImage",
})
// if (blogs.length === 0) throw new BadRequestError("No mor
res.status(StatusCodes.OK).json({
    data: { blogs, page: req.pagination.page, limit: req.pagination.limit },
        ...
        .populate({
        path: "comments",
        options: { sort: { createdAt: -1 } }, // Sort commen
        populate: {
        path: "author",
        select: "name profileImage",
},
...
throw new BadRequestError("Blog not found or you are not")
res.status(StatusCodes.OK).json({
data: { blog },
success: true,
msg: "Blog Fetched Successfully",
...
throw new BadRequestError("Tags should be an array of variable")
...
const deleteBlog = async (req: Request, res: Response) => {
  ...
  const updateBlog = async (req: Request, res: Response) => {
    ...
    if (tags && !Array.isArray(tags))
      throw new BadRequestError("Tags should be an array of values");
    ...
    if (!blog)
      throw new BadRequestError("You are not authorized to update this blog or the blog does not exist");
    ...
    const trendingBlogs = await Blog.aggregate([
      ...
      },
      {
        $project: {
          _id: 1,
          title: 1,
          totalScore: { $add: ["$views", "$likesCount", "$commentsCount"] },
